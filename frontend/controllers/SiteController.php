...
class SiteController extends Controller
{
    ...
    public function __construct($id, $module, PasswordResetService $passwordResetService, ContactService $contactService, SignupService $signupService, array $config = [])
    {
    ...
    public function actionContact()
    {
        ...
        if ($form->load(Yii::$app->request->post()) && $form->validate()) {
            try {
                ...
                Yii::$app->session->setFlash('success', 'Спасибо за отправку нам сообщения. Мы свяжемся с вами в ближайшее время.');
                return $this->goHome();
            } catch (\Exception $e) {
    ...
    public function actionConfirm($token) {
        try {
            ...
            Yii::$app->session->setFlash('success', 'Email адрес успешно подтвержден.');
